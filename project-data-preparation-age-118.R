...
txn_for_profile_age_118 <- txn_for_profile_age_118[order(txn_for_profile_age_118$person_id, 
                                                         txn_for_profile_age_118$offerid, 
                                                         txn_for_profile_age_118$offer_id), ]



txn_only <- subset(txn_for_profile_age_118, event == 'transaction')
...
for(i in 1:nrow(txn_only)) {
  ...
  offer_completed_rows <- txn_for_profile_age_118[txn_for_profile_age_118$event == 'offer completed' &
                                                    txn_for_profile_age_118$person_id == person_id &
                                                    txn_for_profile_age_118$time == time, ]
...
write.csv(txn_with_promotion, file = './txn_with_promotion_age_118.csv')
write.csv(txn_without_promotion, file = './txn_without_promotion_age_118.csv')
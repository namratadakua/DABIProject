...
txn_with_promotion <- read.csv("./txn_with_promotion_age_118.csv")
...
selected_data <- subset(txn_with_profile_portfolio, 
                        select = c('profile_number', 'profolio_number', 'spent_amount',
                                   'reward_paid', 'time', 'became_member_on'))
...
write.csv(txn_with_profile_portfolio[, !names(txn_with_profile_portfolio) %in% c("channels")], 
          "age_118_txn_with_promotion_clustered.csv", 
          row.names = FALSE)
...
save_kable(summary_cluster_wise_kable, "age_118_summary_cluster_wise.png")
...
save_kable(rfm_cluster_summary, "age_118_rfm_summary_cluster_wise.png")
